...
public class JsonUtils {
    ...
    public static Sandwich parseSandwichJson(String json) {
        ...
        try {
            ...
            String name = jsonDetails.optString(JSON_DETAILS_KEY_NAME);
            JSONObject jsonName = new JSONObject(name);

            String mainName = jsonName.optString(JSON_NAME_KEY_MAIN_NAME);
            ...
            for (int i = 0; i < jsonAlsoKnownAs.length(); i++) {
                alsoKnownAs.add(jsonAlsoKnownAs.optString(i));
            }

            String placeOfOrigin = jsonDetails.optString(JSON_DETAILS_KEY_PLACE_OF_ORIGIN);

            String description = jsonDetails.optString(JSON_DETAILS_KEY_DESCRIPTION);

            String image = jsonDetails.optString(JSON_DETAILS_KEY_IMAGE);
